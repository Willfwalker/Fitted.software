...
export function DashboardHeader({ userRole }: DashboardHeaderProps) {
  ...
  const canRequestFeatures = hasPermission(userRole ?? null, "features:request")
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-2">
          {canRequestFeatures && (
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs text-[var(--text-muted)] hover:text-[var(--text)] hover:border-[var(--accent)] transition-colors cursor-pointer"
            >
      ...
      {canRequestFeatures && <ChatPanel open={chatOpen} onOpenChange={setChatOpen} />}
    </>
  )
}
