...
export function CompanyList({ companies, searchQuery, currentSort, uiConfig }: CompanyListProps) {
  ...
  useEffect(() => {
    if (searchParams.get("create") === "true") {
      ...
      router.replace(`/dashboard/crm/companies${params.toString() ? `?${params}` : ""}`)
    }
  }, [searchParams, router])
  ...
  const handleSearch = (value: string) => {
    ...
    router.push(`/dashboard/crm/companies${params.toString() ? `?${params}` : ""}`)
  }
    ...
    <>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        {uiConfig?.filters && uiConfig.filters.length > 0 && (
          <DynamicFilters filters={uiConfig.filters} basePath="/dashboard/crm/companies" />
        )}
                ...
                >
                  ...
                  {uiConfig?.columns?.filter(c => c.visible).map((col) => (
                    <TableCell key={col.key} className="text-[var(--text-muted)] font-light text-[0.85rem]">
                      {col.source === "metadata"
                        ? (company as unknown as Record<string, unknown>).metadata
                          ? String(((company as unknown as Record<string, unknown>).metadata as unknown as Record<string, unknown>)?.[col.column] ?? "—")
                          : "—"
                        : String((company as unknown as Record<string, unknown>)[col.column] ?? "—")}
                    </TableCell>
                  ))}
                  <TableCell>
                    <DropdownMenu>
