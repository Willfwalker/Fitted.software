...
export function ContactForm({ open, onOpenChange, companies, contact, uiConfig }: ContactFormProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[var(--bg-card)] border-[var(--border)] text-[var(--text)] sm:max-w-[480px]">
        ...
        <form action={formAction} className="space-y-4 mt-2">
          ...
          {uiConfig && uiConfig.fields.length > 0 && (
            <DynamicFields config={uiConfig} metadata={contact?.metadata} />
          )}
